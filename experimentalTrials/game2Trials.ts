import fs from "fs";
// START: File storage
const filePath = "experimentalTrials\\game2\\";
/**
...
function secondLilyPad() {
   ...
   if (
      ...
      // win
      return [numberOfSuits, true];
   }

   return [numberOfSuits, undefined];
}
...
while (numGames < numTrials) {
   lastGame = secondLilyPad()
   nextOutput = "\n"
   if (lastGame[1]) { // if the game was won,
      nextOutput += "W";
   } else if (lastGame[1] == false) {
      nextOutput += "L";
   } else { // reject any ties (we are looking for the experimental probability of the next movement being caused due to a win (AKA the experimental probability that the next move is fowards))
      continue;
   }
   
   for (let i in lastGame[0]) {
      nextOutput += "," + lastGame[0][i];
   }
